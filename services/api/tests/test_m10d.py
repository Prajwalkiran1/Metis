...
# ── Tasks ───────────────────────────────────────────────────────────────────
@pytest.mark.asyncio
async def test_hod_assigns_task_to_dept_teacher(client):
    fx = await _build_fixture()
    h = await _login(client, HOD_EMAIL)
    r = await client.post(
        "/workflow/tasks",
        headers=h,
        json={
            "assigned_to_user_id": str(fx.teacher_id),
...
@pytest.mark.asyncio
async def test_hod_cannot_assign_to_outside_teacher(client):
    fx = await _build_fixture()
    h = await _login(client, HOD_EMAIL)
    r = await client.post(
        "/workflow/tasks",
        headers=h,
        json={
            "assigned_to_user_id": str(fx.outside_teacher_id),
            "task_type": "invigilation",
            "title": "Invigilate cross-dept",
        },
    )
...
@pytest.mark.asyncio
async def test_task_accept_then_complete(client):
    fx = await _build_fixture()
    h = await _login(client, HOD_EMAIL)
    r = await client.post(
        "/workflow/tasks",
    ...
    accepted = await client.post(
        f"/workflow/tasks/{task_id}/status",
        headers=t,
...
@pytest.mark.asyncio
async def test_task_decline_requires_reason(client):
    fx = await _build_fixture()
    h = await _login(client, HOD_EMAIL)
    r = await client.post(
        "/workflow/tasks",
    ...
    assert good.json()["decline_reason"] == "on leave that week"


@pytest.mark.asyncio
async def test_only_assigner_can_cancel(client):
    fx = await _build_fixture()
    h = await _login(client, HOD_EMAIL)
    r = await client.post(
        "/workflow/tasks",
